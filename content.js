...
let userPreferences = {
    ...
    gridMode: false, // NEW: Whether to use grid layout (images only with hover info)
};
    ...
    return new Promise((resolve) => {
        try {
            chrome.storage.sync.get([
                ...
                `${STORAGE_KEY_PREFIX}grid_mode`  // NEW: Load grid mode
            ], function(result) {
                ...
                if (result[`${STORAGE_KEY_PREFIX}grid_mode`] !== undefined) {  // NEW: Check for grid mode
...
function saveUserPreferences() {
    try {
        const data = {
            ...
            [`${STORAGE_KEY_PREFIX}grid_mode`]: userPreferences.gridMode  // NEW: Save grid mode
        };
...
/**
 * Create and open the settings overlay
 */
function createSettingsOverlay() {
  ...
  `;
    ...
    displaySection.appendChild(gridRow);  // NEW: Add grid mode row
  ...
  `;
    ...
    saveButton.addEventListener('click', () => {
        ...
        const newGridMode = document.getElementById('grid-toggle').checked;  // NEW: Get grid mode value

        // Update preferences
        userPreferences.startDay = newStartDay;
        userPreferences.hideEmptyDays = newHideEmpty;
        userPreferences.compactMode = newCompactMode;
        userPreferences.gridMode = newGridMode;  // NEW: Update grid mode preference
...
function renderCalendar(schedule) {
    ...
    calendarTitle.textContent = 'Weekly Schedule';

    // Settings button che ora apre l'overlay
